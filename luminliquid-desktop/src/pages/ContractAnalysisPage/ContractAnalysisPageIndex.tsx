...
import EVMContractAnalysisPanel from "./EVMContractAnalysisPanel";
import SolanaContractAnalysisPanel from "./SolanaContractAnalysisPanel";
import SuiContractAnalysisPanel from "./SuiContractAnalysisPanel";
import AptosContractAnalysisPanel from "./AptosContractAnalysisPanel";

interface ContractAnalysisPageIndexProps {
    children?: React.ReactNode;
}

interface ContractAnalysisPageIndexState {
    theme: 'dark' | 'light';
    ...
    currentFile: string;
    isCollapsed: boolean;
    activeRightPanel: 'evm' | 'solana' | 'sui' | 'aptos'; // 新增状态
}
...
class ContractAnalysisPageIndex extends React.Component<ContractAnalysisPageIndexProps, ContractAnalysisPageIndexState> {
...
contract UniswapV3Router {
    ...
    constructor(props: ContractAnalysisPageIndexProps) {
        super(props);
        ...
        this.state = {
            ...
            isCollapsed: false,
            activeRightPanel: 'evm' // 默认显示函数分析面板
        };
    }
    ...
    private handleNetworkSelect = (networkKey: string, subKey?: string) => {
        this.setState({ selectedNetwork: networkKey });
        let activeRightPanel: 'evm' | 'solana' | 'sui' | 'aptos' = 'evm';
        switch (networkKey) {
            case 'evm':
                activeRightPanel = 'evm';
                break;
            case 'solana':
                activeRightPanel = 'solana';
                break;
            case 'sui':
                activeRightPanel = 'sui';
                break;
            case 'aptos':
                activeRightPanel = 'aptos';
                break;
            default:
                activeRightPanel = 'evm';
        }
        this.setState({ activeRightPanel });
    };
    ...
    private renderNetworkMenu = () => {
        const { theme, selectedNetwork, isCollapsed } = this.state;
        ...
        if (isCollapsed) {
            return (
                <div style={{ padding: '8px 0' }}>
                    {this.networkMenu.map((menu) => (
                        <div
                            key={menu.key}
                            onClick={() => this.handleNetworkSelect(menu.key)}
                            className={`menu-item menu-item-hover ${selectedNetwork === menu.key ? 'menu-item-selected' : ''} collapsed-menu-item`}
                            style={{
                                display: 'flex',
                                alignItems: 'center',
                                padding: '12px 8px',
                                ...
                                fontWeight: '500',
                                cursor: 'pointer',
                                backgroundColor: selectedNetwork === menu.key ?
                                    (theme === 'dark' ? '#3C4858' : '#E1E5E9') : 'transparent',
                                borderRight: selectedNetwork === menu.key ? `3px solid ${primaryColor}` : '3px solid transparent',
                                color: selectedNetwork === menu.key ?
                        ...
                        >
                            <Icon
                                ...
                                size={16}
                                className="menu-icon"
                                color={selectedNetwork === menu.key ? primaryColor : undefined}
                            />
                        </div>
                    ))}
                </div>
            ...
            <div style={{ padding: '12px 0' }}>
                {this.networkMenu.map((menu) => (
                    <div key={menu.key} style={{ marginBottom: '4px' }}>
                        <div
                            onClick={() => this.handleNetworkSelect(menu.key)}
                            className={`menu-item menu-item-hover ${selectedNetwork === menu.key ? 'menu-item-selected' : ''}`}
                            style={{
                                display: 'flex',
                                alignItems: 'center',
                                gap: '10px',
    ...
    private renderRightPanel = () => {
        const { theme, selectedFunction, activeRightPanel } = this.state;
        switch (activeRightPanel) {
            case 'evm':
                return (
                    <EVMContractAnalysisPanel
                        theme={theme}
                        selectedFunction={selectedFunction}
                        functions={this.contractData.functions}
                        onFunctionSelect={this.handleFunctionSelect}
                    />
                );
            case 'solana':
                return (
                    <SolanaContractAnalysisPanel
                        theme={theme}
                        selectedFunction={selectedFunction}
                        functions={this.contractData.functions}
                        onFunctionSelect={this.handleFunctionSelect}
        ...
        return (
            ...
            >
                ...
                <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
                    {/* Left Panel - Contract Overview */}
                    {this.renderLeftPanel()}

                    {/* Center Panel - Source Code */}
                    {this.renderCenterPanel()}

                    {/* Right Panel - Function Analysis */}
...
export default ContractAnalysisPageIndex;